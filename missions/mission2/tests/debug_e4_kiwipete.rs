use mission2_movegen::board::{Board, Square, PieceType, Color};
...
fn debug_e4_pawn_kiwipete() {
    ...
    // Filter for E4 moves
    let e4_moves: Vec<_> = all_moves.moves().iter()
    ...
    let e4_pseudo: Vec<_> = pseudo_legal.moves().iter()
        .filter(|m| m.from() == Square::E4)
        .collect();
    ...
    if !e4_pseudo.is_empty() {
        ...
        println!("White in check: {}", movegen.is_in_check(&test_board, Color::White));
    }
}
