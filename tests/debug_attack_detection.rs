use prawn::board::{Board, Color, PieceType, Square};
...
fn debug_rook_attack_detection() {
    ...
    println!(
        "Rook bitboard: {:064b}",
        board.piece_bitboard(Color::Black, PieceType::Rook)
    );
...
fn debug_after_pawn_moves() {
    ...
    assert!(
        !white_in_check,
        "White king should NOT be in check (pawn still blocks on E5)"
    );
}
